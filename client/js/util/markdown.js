...
class EntityPermalinkWrapper extends BaseMarkdownWrapper {
    ...
    preprocess(text) {
        ...
        text = text.replace(
            ...
            (match, lookbehind, prefix, capture) => `${lookbehind}[${prefix}${unescapeHtml(prefix === "+" ? capture : this.getPrettyName(capture))}](${prefix}${uri.escapeParam(unescapeHtml(capture))})`
        );
        // post number links
        text = text.replace(
            /(^|\s)(@\d+\b)/umg,
...
function modifyLink(token) {
    ...
    if ((match = /^@(\d+)$/.exec(token.href))) {
        ...
        const capture = uri.unescapeParam(match[2]).replace(/\\(.)/g, "$1");
